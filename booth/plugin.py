...
Pibooth plugin for oi-booth. Provides:
  ...
  - Attract slideshow during idle
  - Event photo routing to per-event subdirectories
  - LED on/off/blink control via IPC
...
Config (pibooth.cfg):
  ...
  attract_delay     = 8           # seconds between attract slides (0 to disable)
"""
...
try:
    from booth.ipc import (
        read_command, write_status, write_led_state,
        ACTIVE_EVENT_FILE,
    )
    from booth.modes import make_gif, make_boomerang
...
def _apply_led(app, led_name: str, state: str):
    ...
    if leds_board is None:
        print(f"[oi-booth] LED mock: {led_name} → {state}")
        return
    targets = ["capture", "print"] if led_name == "all" else [led_name]
    for name in targets:
...
def _build_qr_surface(url: str, win_size: tuple):
    ...
    draw.text(((panel_w - (bbox[2] - bbox[0])) // 2, pad // 2),
              label, font=font_label, fill=(255, 255, 255, 255))
    panel.paste(qr_pil, (pad, 36))

    short_url = url.replace("http://", "")
    bbox2 = draw.textbbox((0, 0), short_url, font=font_url)
    draw.text(((panel_w - (bbox2[2] - bbox2[0])) // 2, 36 + qr_size + 8),
              short_url, font=font_url, fill=(180, 180, 180, 255))
...
def _copy_to_event(picture_file: str):
    """Copy a finished photo into the active event's subdirectory."""
    if not ACTIVE_EVENT_FILE.exists():
        return
    try:
        event = json.loads(ACTIVE_EVENT_FILE.read_text())
    ...
    try:
        ...
        shutil.copy2(picture_file, dest)
...
def pibooth_configure(cfg):
    ...
    cfg.add_option(SECTION, "attract_delay", DEFAULTS["attract_delay"], "Seconds between attract slideshow slides (0 to disable)")


@hookimpl
def pibooth_startup(cfg, app):
    global _current_mode, _attract_idx, _attract_timer
    ...
    print(f"[oi-booth] AI backgrounds: {'enabled' if AI_BG_SUPPORTED else 'disabled (Pi 4+ required)'}")
    ...
    try:
        ...
        if count:
            print(f"[oi-booth] Attract slideshow: {count} image(s) in {get_attract_dir()}")
        else:
            print(f"[oi-booth] Attract slideshow: no images — skipping")
    except Exception as e:
        print(f"[oi-booth] Attract init error: {e}")
...
def state_wait_do(cfg, app, win, events):
    """Poll for CoPilot commands and drive the attract slideshow."""
    global _current_mode, _current_state, _attract_idx, _attract_timer
    _current_state = "wait"
    write_status("wait", mode=_current_mode, photo_count=_photo_count,
                 last_session_id=_last_session_id)

    # ── CoPilot IPC commands ──────────────────────────────────────────────────
    cmd = read_command()
    if cmd:
    ...
    if delay > 0:
        ...
        if now - _attract_timer >= delay:
            try:
                ...
                print(f"[oi-booth] Attract error: {e}")
            _attract_timer = now
...
def state_processing_enter(cfg, app, win):
    """AI background removal, GIF assembly, and event photo routing."""
